 ...
 *
 */
public class ObjectResultsWithTarget extends BasicName implements ConditionalResult {
	...
	public ObjectResultsWithTarget() {
		super();
		move = TunnelType.Here;
		checkSubmembraneProp = false;
	}

	public ObjectResultsWithTarget(TunnelType move) {
		this.move = move;
		checkSubmembraneProp = false;
	}
	...
	@Override
	public ObjectResultsWithTarget deepClone() {
		try {
			// MemPropertyCondition没必要clone
			ObjectResultsWithTarget cloned = (ObjectResultsWithTarget) super.clone();
	...
	public Tunnel selectTunnel(Membrane current) throws TunnelNotExistException {
		...
		case Go_one_of_specified:
			...
			if (tunnel == null) {
				List<Membrane> scope;
				if (move == TunnelType.In || move == TunnelType.In_all_of_specified
						|| move == TunnelType.In_one_of_specified) {
	...
	public List<ObjectResult> getObjectResults() {
		return ors;
	}

	public boolean isTheSameTarget(ObjectResultsWithTarget ort) {
		if (move == ort.move && getTargetsName().equals(ort.getTargetsName())) {
			return true;
		} else {
			return false;
		}
	...
	public void combine(ObjectResultsWithTarget ort) {
		for (ObjectResult or : ort.ors)
			addObjectResult(or);
		ort.ors.clear();
	}
